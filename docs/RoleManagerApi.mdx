...
The RoleManager provides an interface for defining operations to manage roles. Adding a matching function to the RoleManager enables the use of wildcards in role names and domains.

### `AddNamedMatchingFunc()`

The `AddNamedMatchingFunc` function adds a `MatchingFunc` by Ptype to the RoleManager. The `MatchingFunc` is used when performing role matching.
...
The `AddNamedDomainMatchingFunc` function adds a `DomainMatchingFunc` by Ptype to the RoleManager. The `DomainMatchingFunc` is similar to the `MatchingFunc` above.
...
The `GetRoleManager` function retrieves the current role manager for `g`.
...
The `GetNamedRoleManager` function retrieves the role manager by named Ptype.
...
The `Clear` function removes all stored data and resets the role manager to its initial state.
