...
The `[role_definition]` section defines RBAC role inheritance relationships. Casbin supports multiple independent RBAC systems—users can have roles with inheritance relationships, and resources can have their own role hierarchies. These two RBAC systems won't interfere with each other.

The `[role_definition]` section defines RBAC role inheritance relationships. Casbin supports multiple independent RBAC systems—users can have roles with inheritance relationships, and resources can have their own role hierarchies. These two RBAC systems won't interfere with each other.

This section is optional. If you don't use RBAC roles in the model, you can omit this section.
...
The above role definition shows that `g` is an RBAC system, and `g2` is another RBAC system. `_,_` means there are two parties involved in an inheritance relation. In the most common case, you usually use `g` alone if you only need roles for users. You can also use both `g` and `g2` when you need roles (or groups) for both users and resources. See the [rbac_model.conf](https://github.com/casbin/casbin/blob/master/examples/rbac_model.conf) and [rbac_model_with_resource_roles.conf](https://github.com/casbin/casbin/blob/master/examples/rbac_with_resource_roles_model.conf) for examples.
...
It means that `alice` inherits or is a member of the role `data2_admin`. Here, `alice` can be a user, a resource, or a role. Casbin only recognizes it as a string.
...
2. Casbin doesn't verify whether a user is a valid user or a role is a valid role. That should be handled by authentication.
