...
description: Comprehensive guide to Casbin model configuration syntax and structure
keywords: [model syntax, configuration, policy definition, matchers, access control]
authors: [nodece]
---

A Casbin model configuration (`.conf` file) must include at least four core sections: `[request_definition]`, `[policy_definition]`, `[policy_effect]`, and `[matchers]`.

Role-Based Access Control (RBAC) models require an additional `[role_definition]` section.

Model configurations support comments, which begin with the `#` symbol. Example:

```ini
# This is a comment
[request_definition]
r = sub, obj, act  # Parameters for enforcement request
```

All text following `#` is treated as a comment and ignored during parsing.

## Request Definition

The `[request_definition]` section defines the parameter structure for the enforcement function call `e.Enforce(...)`.
...
In this example, `sub`, `obj`, and `act` represent the classic access control triple: subject (accessing entity), object (target resource), and action (operation type). The request format can be customized to include additional parameters as needed. For example, you can use `sub, act` if you don't need to specify a particular resource, or `sub, sub2, obj, act` if you have two accessing entities.

## Policy Definition

The `[policy_definition]` section defines the structure and format of policy rules. For example, we have the following model:
...
Each line in a policy is called a policy rule. Each policy rule starts with a `policy type`, such as `p` or `p2`. It is used to match the policy definition if there are multiple definitions. The above policy shows the following binding. These bindings are used in the matchers.
...
All elements in policy rules are treated as string values. This includes numbers and special characters, which will be processed as string literals. If you have any questions about this, please refer to the discussion at: [https://github.com/casbin/casbin/issues/113](https://github.com/casbin/casbin/issues/113)
...
This policy effect implements an allow-override mechanism: if any matched policy rule grants `allow`, access is permitted. `p.eft` represents the policy effect, which can be either `allow` or `deny` (default: `allow`). It is optional, and the default value is `allow`. Since we didn't specify it above, it uses the default value.
...
Although we designed the syntax of the policy effect as above, the current implementations primarily use predefined policy effects. This is because we found that there isn't much need for that level of flexibility. So for now, you must use one of the built-in policy effects instead of customizing your own.

:::

The supported built-in policy effects are:

| Policy Effect Expression                                    | Behavior              | Example                                             |
|--------------------------------------------------------------|-----------------------|-----------------------------------------------------|
| `some(where (p.eft == allow))`                             | Allow-override        | [ACL, RBAC models](/docs/supported-models#examples)  |
...
| `priority(p.eft) || deny`                                   | Priority-based        | [Priority](/docs/supported-models#examples)         |
| `subjectPriority(p.eft)`                                   | Role-based priority   | [Subject-Priority](/docs/supported-models#examples) |
...
Arithmetic operators (+, -, *, /) and logical operators like `&&`, `||`, `!` can be used in matchers. For optimal performance:
- Place frequently matching conditions first
- Order expressions from most specific to most general
- Avoid expensive operations in matchers for high-throughput scenarios
...
```go
// Example demonstrating performance impact of matcher expression order
const rbacModels = `
[request_definition]
r = sub, obj, act
...
func TestManyRoles(t *testing.T) {
    ...
    // Create 2500 project-specific roles and resources
    for projectNumber := 1; projectNumber < 2500; projectNumber++ {
        // Create role-resource permission mappings for each project
        for _, role := range roles {
            roleName := fmt.Sprintf("%s_project:%d", role, projectNumber)
            resourcePath := fmt.Sprintf("/projects/%d", projectNumber)
            e.AddPolicy(roleName, resourcePath, "GET")  // Grant GET permission for the role on the resource
        }
        jasmineRoleName := fmt.Sprintf("%s_project:%d", roles[1], projectNumber)
        e.AddGroupingPolicy("jasmine", jasmineRoleName)  // Assign project-specific role to user
    }
    ...
    // - User 'abu' has minimal roles (2 total)
    // - User 'jasmine' has extensive role assignments (2500 total)
    // This demonstrates how matcher expression ordering significantly impacts performance with large role sets. The first evaluation suffers from O(n) complexity with many role assignments, while subsequent calls leverage caching for better performance.
...
The matcher expression order significantly impacts performance. In this example, the first evaluation for user 'jasmine' takes over 6 seconds due to the large number of role assignments, while subsequent requests benefit from internal caching mechanisms.

```bash
# Execute the performance test
GODEBUG=cgocheck=0 go test -run ^TestManyRoles$ github.com/casbin/casbin/v2 -v
