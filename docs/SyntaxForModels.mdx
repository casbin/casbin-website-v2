...
- A model configuration (CONF) can contain comments. Comments start with the `#` symbol, and everything following the `#` symbol will be treated as a comment.
...
``[policy_effect]`` defines the policy effect. It determines whether the access request should be approved if multiple policy rules match the request. For example, one rule permits while another denies.
...
This means that if there are no matched policy rules of ``deny``, the final effect is ``allow`` (also known as deny-override). ``some`` means at least one matched policy rule exists. ``any`` means all matched policy rules (not used here). The policy effect can even be connected with logical expressions:
...
This means that there must be at least one matched policy rule of ``allow``, and there cannot be any matched policy rule of ``deny``. In this way, both allow and deny authorizations are supported, and deny overrides allow.

:::note

Although we designed the syntax of the policy effect as above, the current implementations only use hard-coded policy effects. This is because we found that there isn't much need for that level of flexibility. For now, you must use one of the built-in policy effects instead of customizing your own.
...
**Separation of Duties (sod)** - Prevents a user from holding two conflicting roles simultaneously. When Alice is assigned `finance_requester`, the system blocks any attempt to also assign her `finance_approver`.
...
**Prerequisite Role (rolePre)** - Requires users to have one role before they can receive another. Nobody gets `db_admin` access unless they already have the `security_trained` role first.
...
``[matchers]`` defines the policy matchers. The matchers are expressions that define how policy rules are evaluated against the request.
...
The above matcher is the simplest and means that the subject, object, and action in a request should match the corresponding fields in a policy rule.
...
You can also use the "in" operator, which is the only operator with a text name. This operator checks whether the array on the right-hand side contains a value that is equal to the value on the left side. Equality is determined using the == operator, and this library does not check types between the values. As long as two values can be cast to interface{} and can still be checked for equality with ==, they will behave as expected. Note that you can use a parameter for the array, but it must be an []interface{}.
...
The matcher evaluation in Casbin is implemented by expression evaluators in each language. Casbin integrates these to provide the unified PERM language. In addition to the model syntax provided here, these expression evaluators may offer extra functionality that might not be supported by another language or implementation. Please be cautious when using this functionality.
...
If you encounter a performance issue with Casbin, it is likely caused by the low efficiency of the expression evaluator. You can address the issue to Casbin or the expression evaluator directly for advice on improving performance. For more details, please refer to the [Benchmarks](/docs/benchmark) section.
