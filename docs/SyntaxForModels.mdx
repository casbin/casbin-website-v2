...
### Domain Parameters in Matchers

When working with RBAC with domains, you can use any parameter name for the domain token. Casbin automatically detects domain parameters in matchers when using domain-aware functions like `keyMatch`:
...
In this example, `tenant` is used instead of the conventional `dom`. The matcher will correctly evaluate domain-based policies as long as the parameter names match your request and policy definitions.
