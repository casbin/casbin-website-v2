...
For example, to implement a temporary role policy, you can use the following approach:
...
Add a conditional function for the role policy (`g` type policy) through `AddNamedLinkConditionFunc`. When enforcement is executed, the corresponding parameters are automatically obtained and passed to the conditional function for checking. If the check passes, the corresponding role policy (`g` type policy) is valid; otherwise, it is invalid.
...
Custom condition functions must conform to the following function type:
...
As with the basic `ConditionalRoleManager`, custom functions are supported, and there is no difference in usage.

Note that `DomainMatchingFunc`, `MatchingFunc`, and `LinkConditionFunc` are at different levels and are used in different scenarios.
