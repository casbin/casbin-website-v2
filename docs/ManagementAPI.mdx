...
This is the core API that provides full support for Casbin policy management.

## Filtered API

Almost all filtered APIs have the same parameters `(fieldIndex int, fieldValues ...string)`. `fieldIndex` specifies the starting index for matching, `fieldValues` specifies the values that results must contain. Note: An empty string in `fieldValues` matches any value.
...
EnforceWithMatcher uses a custom matcher to decide whether a "subject" can access an "object" with the operation "action", input parameters are usually: (matcher, sub, obj, act), use model matcher by default when matcher is "".
...
let areRulesRemoved = e.remove_named_grouping_policies("g", rules).await?;
```
