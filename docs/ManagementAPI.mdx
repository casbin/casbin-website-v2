...
```go
request := []interface{}{"alice","book","read"}
ok, err := e.Enforce(request...) // true,nil
```
...
request := []interface{}{"alice","book","read"}
ok, err := e.EnforceWithMatcher(matcher, request...) // true,nil
```
...
```go
request := []interface{}{"alice","book","read"}
ok, reason, err := e.EnforceEx(request...) // true, [alice book read] , nil
```
...
matcher := "r.sub == p.sub && r.obj == p.obj && r.act == p.act"
request := []interface{}{"alice","book","read"}
ok, reason, err := e.EnforceExWithMatcher(matcher, request...) //true, [alice book read], nil
```
...
```go
requests := [][]interface{}{{"alice","book","read"},{"bob","book","write"}}
boolArray, err := e.BatchEnforce(requests) // [true true] ,nil
```
...
```go
allSubjects := e.GetAllSubjects() // [alice bob]
```
...
```go
allNamedSubjects := e.GetAllNamedSubjects("p") // [alice bob]
```
...
```go
allObjects := e.GetAllObjects() // [book pen]
```
...
```go
allNamedObjects := e.GetAllNamedObjects("p") // [bob pen]
```
...
```go
allActions := e.GetAllActions() // [read write get]
```
...
```go
allNamedActions := e.GetAllNamedActions("p") // [read write get]
```
...
```go
allRoles = e.GetAllRoles() // [] Incase of RBAC type configs, roles are populated as an array of strings with the defined roles
```
...
```go
allNamedRoles := e.GetAllNamedRoles("g")  // [] Returns all roles with the type of role specified
```
...
```go
policy = e.GetPolicy() // [[alice book read] [bob book read] [bob book write] [alice pen get] [bob pen get]]
```
...
```go
filteredPolicy := e.GetFilteredPolicy(0, "alice") // [[alice book read] [alice pen get]]
```
...
```go
namedPolicy := e.GetNamedPolicy("p") // [[alice book read] [bob book read] [bob book write] [alice pen get] [bob pen get]]
```
...
```go
filteredNamedPolicy = e.GetFilteredNamedPolicy("p", 0, "bob") // [[bob book read] [bob book write] [bob pen get]]
```
...
```go
groupingPolicy := e.GetGroupingPolicy() // [[alice admin] [bob user]]
```
...
```go
filteredGroupingPolicy := e.GetFilteredGroupingPolicy(0, "alice") // [[alice admin]]
```
...
```go
namedGroupingPolicy := e.GetNamedGroupingPolicy("g") // [[alice admin] [bob user]]
```
...
```go
namedGroupingPolicy := e.GetFilteredNamedGroupingPolicy("g", 0, "alice") // [[alice admin]]
```
...
```go
hasPolicy := e.HasPolicy("data2_admin", "data2", "read") // true/false
```
...
```go
hasNamedPolicy := e.HasNamedPolicy("p", "data2_admin", "data2", "read") // true/false
```
...
```typescript
const hasNamedPolicy = await e.hasNamedPolicy(
  "p",
  "data2_admin",
  "data2",
  "read"
...
```go
added := e.AddPolicy('eve', 'data3', 'read') // true if entry added and false if already exists
```
...
areRulesAdded := e.AddPolicies(rules) // true if all rules are added, false if any entry is inconsistent
```
...
```go
added := e.AddNamedPolicy("p", "eve", "data3", "read") // Same as AddPolicy
```
...
areRulesAdded := e.AddNamedPolicies("p", rules) // same as addPolicies
```
...
```go
removed, err := e.RemovePolicy("alice", "data1", "read") // true/false,nil else error
```
...
areRulesRemoved, err := e.RemovePolicies(rules) // true/false, nil
```
...
```go
removed, err := e.RemoveFilteredPolicy(0, "alice", "data1", "read") // true/false , nil
```
...
```go
removed, err := e.RemoveNamedPolicy("p", "alice", "data1", "read") // true/false, nil
```
...
areRulesRemoved, err := e.RemoveNamedPolicies("p", rules) // true/false, nil
```
...
```go
removed, err := e.RemoveFilteredNamedPolicy("p", 0, "alice", "data1", "read") // true/false, nil
```
...
```go
has := e.HasGroupingPolicy("alice", "data2_admin") // true/false
```
...
```go
has := e.HasNamedGroupingPolicy("g", "alice", "data2_admin") //true/false
```
...
```go
added, err := e.AddGroupingPolicy("group1", "data2_admin") // true/false, nil
```
...
areRulesAdded, err := e.AddGroupingPolicies(rules) // true/false, err
```
...
```go
added, err := e.AddNamedGroupingPolicy("g", "group1", "data2_admin") //true/false, nil
```
...
areRulesAdded,err := e.AddNamedGroupingPolicies("g", rules) // true/false,nil
```
...
```go
removed,err := e.RemoveGroupingPolicy("alice", "data2_admin") //true/false, nil
```
...
areRulesRemoved, err := e.RemoveGroupingPolicies(rules) // true/false, nil
```
...
```go
removed,err := e.RemoveFilteredGroupingPolicy(0, "alice") // true/false, err
```
...
```go
removed,err := e.RemoveNamedGroupingPolicy("g", "alice") // true/false,nil
```
...
areRulesRemoved, err := e.RemoveNamedGroupingPolicies("g", rules) // true/false,err
```
...
```go
removed, err := e.RemoveFilteredNamedGroupingPolicy("g", 0, "alice") //true/false,err
```
...
```go
updated, err := e.UpdatePolicy([]string{"eve", "data3", "read"}, []string{"eve", "data3", "write"}) // true/false, nil
```
...
```typescript
const update = await e.updatePolicy(
  ["eve", "data3", "read"],
  ["eve", "data3", "write"]
);
```
...
```go
updated, err := e.UpdatePolicies([][]string{{"eve", "data3", "read"}, {"jack", "data3", "read"}}, [][]string{{"eve", "data3", "write"}, {"jack", "data3", "write"}}) // true/false, err
```
...
```go
err := e.LoadFilteredPolicy() // error
```
...
```go
err := e.LoadIncrementalFilteredPolicy() // error
```
...
```go
succeed, err : = e.UpdateGroupingPolicy([]string{"data3_admin", "data4_admin"}, []string{"admin", "data4_admin"}) // true/false, error
```
...
```go
succeed, err : = e.UpdateNamedGroupingPolicy("g1",[]string{"data3_admin", "data4_admin"}, []string{"admin", "data4_admin"}) // true/false, error
```
