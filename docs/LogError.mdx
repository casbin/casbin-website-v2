...
:::note
**Python:** PyCasbin uses the standard `logging` module. Configure your app’s logging as usual; PyCasbin uses `logging.getLogger()`. Optional `logging_config` is documented in [pycasbin/util/log.py](https://github.com/casbin/pycasbin/blob/master/casbin/util/log.py). See [Python logging](https://docs.python.org/3/library/logging.config.html) and [Django logging](https://docs.djangoproject.com/en/4.2/topics/logging/) for setup.
:::
