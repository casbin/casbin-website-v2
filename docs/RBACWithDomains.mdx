...
For domain-related API methods like `GetAllUsersByDomain()`, you only need to call `SetFieldIndex()` if the domain parameter is not in the standard second position:
