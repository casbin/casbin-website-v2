...
When using pattern matching functions like `keyMatch` for domain matching, Casbin automatically detects your custom domain token name from the model definition. This works seamlessly with domain matching functions:
...
The domain token must be the second parameter (index 1) in both request and policy definitions for proper detection. No additional configuration is needed—Casbin dynamically determines the token name from your model.
