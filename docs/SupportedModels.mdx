...
10. **[RESTful](https://en.wikipedia.org/wiki/Representational_state_transfer)**: Supports paths like "/res/*", "/res/:id", and HTTP methods like "GET", "POST", "PUT", "DELETE".
11. **IP Match**: Supports IP address matching for network-based access control.
12. **Deny-override**: Both allow and deny authorizations are supported, where deny overrides allow.
13. **Priority**: The policy rules can be prioritized, similar to firewall rules.
