...
```go
res := e.GetRolesForUser("abc") // [admin staff] , nil
```
...
```go
res := e.GetUsersForRole("admin") // [amber abc] nil
```
...
```go
res := e.HasRoleForUser("amber", "staff")  // false, nil
```
...
```go
e.AddRoleForUser("amber", "staff") // true nil
```
...
e.AddRolesForUser("alice", roles) // true nil
```
...
```go
e.DeleteRoleForUser("abc", "staff") // true nil
```
...
```go
e.DeleteRolesForUser("abc") // true nil
```
...
```go
e.DeletePermission("read") // true, nil
```
...
```go
e.AddPermissionForUser("employee", "read") // true,nil
```
...
for i := 0; i < len(permissions); i++ {
    e.AddPermissionsForUser("amber", permissions[i]) // true,nil
}
```
...
```go
e.DeletePermissionForUser("bob", "read") // false,nil
```
...
```go
e.DeletePermissionsForUser("bob") // false,nil (User does not Exist)
```
...
```go
e.GetPermissionsForUser("admin") //[[admin data1 read] [admin data1 write]] , nil
```
...
```go
e.HasPermissionForUser("admin", []string{"read"}) // true
```
...
```go
e.GetImplicitRolesForUser("amber")
```
...
GetUsersForRole("staff") can only get: ["admin"].  
But GetImplicitUsersForRole("staff") will get: ["admin", "amber"].
...
```go
users := e.GetImplicitUsersForRole("user") // ["abc","amber","admin"]
```
...
```go
e.GetImplicitPermissionsForUser("alice") //[[admin data1 read] [admin data1 write] [staff data2 read] [staff data2 write]] <nil>
```
...
```go
resources, err := e.GetImplicitResourcesForUser("alice") // [[amber data1 read] [amber data1 write] [amber data2 read] [amber data2 write]] <nil>
```
