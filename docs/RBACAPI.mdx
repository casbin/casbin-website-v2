...
The **RBAC API** is a convenience layer for role-based operations (roles for user, users for role, permissions, etc.). It is a subset of the [Management API](/docs/management-api) aimed at RBAC use cases.
...
Grants multiple permissions to a user or role.
...
Revokes all permissions from a user or role.
...
Returns all users with access to a resource through a named policy, including those with implicit permissions through roles.
