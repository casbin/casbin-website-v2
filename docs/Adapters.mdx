...
In Casbin, the policy storage is implemented as an adapter (also known as middleware for Casbin). A Casbin user can use an adapter to load policy rules from a storage (also known as `LoadPolicy()`), or save policy rules to it (also known as `SavePolicy()`). To keep light-weight, we don't put adapter code in the main library.
...
- Data Structure. Adapter should support reading at **_least_** six columns.
