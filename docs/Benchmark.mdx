...
The overhead of policy enforcement of [Pycasbin](https://github.com/casbin/pycasbin) is benchmarked in
[`tests/benchmarks`](https://github.com/casbin/pycasbin/tree/master/tests/benchmarks) directory. The testbed for these benchmarks is:
...
Here is the benchmarking result of executing `casbin_benchmark` (op = an `enforce()` call, ms = millisecond):
