...
The `Effect` type represents the result of individual policy rule evaluations, while the `Effector` interface coordinates merging these results into a final authorization decision.
...
The code above demonstrates parameter usage for `MergeEffects()`, which processes effects and matches according to the `expr`.
