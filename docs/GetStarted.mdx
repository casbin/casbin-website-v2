...
Casbin4D comes in a package (currently for Delphi 10.3 Rio) and you can install it in the IDE. However, there are no visual components, which means you can use the units independently of packages. Just import the units in your project (assuming you don't mind the number of them).
...
There are two configuration files: `model.conf` and `policy.csv`. The `model.conf` file stores the access model, while `policy.csv` stores the specific user permission configuration. Using Casbin is straightforward. You only need to create one main structure: the **enforcer**. When constructing this structure, the `model.conf` and `policy.csv` files will be loaded.

In other words, to create a Casbin enforcer, you need to provide a [Model](supported-models) and an [Adapter](adapters).

Casbin provides a [FileAdapter](adapters#file-adapter-built-in) that you can use. See [Adapter](adapters) for more information.

- Example of using the Model file and the default [FileAdapter](adapters#file-adapter-built-in):
...
:::info GraalVM Native Image Note
