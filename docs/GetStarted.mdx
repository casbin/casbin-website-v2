...
// If you use async-std as your async executor
cargo add async-std

// If you use tokio as your async executor, ensure you activate its `macros` feature
cargo add tokio
```
...
Casbin utilizes configuration files to define the access control model.

There are two configuration files: `model.conf` and `policy.csv`. `model.conf` stores the access model, while `policy.csv` stores the specific user permission configuration. Using Casbin is very straightforward. You only need to create one main structure: the **enforcer**. When constructing this structure, you'll load `model.conf` and `policy.csv`.

In other words, to create a Casbin enforcer, you need to provide a [Model](supported-models) and an [Adapter](adapters).

Casbin offers a built-in [FileAdapter](adapters#file-adapter-built-in) for basic file-based policy storage. Refer to the [Adapter documentation](adapters) for additional storage options.

- Example: Creating an enforcer with a model file and the default [FileAdapter](adapters#file-adapter-built-in):
