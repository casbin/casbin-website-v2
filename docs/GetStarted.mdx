...
Using Casbin involves a simple workflow: initialize an **Enforcer** instance with these two configuration files during setup. Technically, an Enforcer requires two components: a [Model](supported-models) definition (access control structure) and an [Adapter](adapters) (policy storage mechanism).
