...
You need two configuration files: `model.conf` and `policy.csv`. The **model** file defines your access control model; the **policy** file holds the permission rules. In practice you work with a single component—the **Enforcer**—which loads both files when it is created.

To create an Enforcer, provide a [Model](/docs/supported-models) and an [Adapter](/docs/adapters).
...
Add an enforcement check in your code immediately before the resource is accessed:
...
For more examples, see [Management API](/docs/management-api) and [RBAC API](/docs/rbac-api). You can also browse the test cases in each language repository.
