...
Before reading, note that some tutorials cover Casbin's model and apply to all language implementations. Some other tutorials are language-specific.

### Our Papers

- [PML: An Interpreter-Based Access Control Policy Language for Web Services](https://arxiv.org/abs/1903.09756)

This paper explores Casbin's design details in depth. Please cite the following BibTex if you use Casbin/PML as a reference in your paper:
...
This extended paper was published in the Journal of Software. The citation for different formats (Refworks, EndNote, etc.) can be found at: [(another version) Access Control Policy Specification Language Based on Metamodel (in Chinese)](https://kns.cnki.net/kcms/detail/Detail.aspx?dbname=CJFDLAST2020&filename=RJXB202002012&v=)
