...
# Kubernetes + Casbin Plugin: K8s-Gatekeeper
...
Casbin K8s-GateKeeper is developed and maintained by Casbin community. Repository of this project is here. <https://github.com/casbin/k8s-gatekeeper>
...
K8s-gatekeeper is an admission webhook for k8s, using [Casbin](/docs/overview) to apply arbitrary user-defined access control rules to help prevent any operation on k8s which administrator doesn't want.

Casbin is a powerful and efficient open-source access control library. It provides support for enforcing authorization based on various access control models. For more detail about Casbin, see [Overview](/docs/overview).
...
(If you do not understand what is Casbin enforcer, model or policy, see this document see: [Get Started](/docs/get-started))
...
Note: these methods are only for user to try K8s-gatekeeper, and it is not secure. If you want to use it in productive environment, please make sure you read  [Chapter 5. Advanced setting](#5-advanced-settings) and make modifications accordingly when necessary before installation
...
See [Chapter 2.1.1](#211-step-1-build-image)
#### 2.3.2 helm install
...
First of all, you are supposed to know the basic grammar of Casbin Models and Policies. If you haven't acknowledged it, please read [Get Started](/docs/get-started) first. In this chapter we will assume that you have known what are Casbin Models and Policies.
...
If you don't know what is casbin funtion, see [Function](/docs/function) for more information.
...
Copy your system's openssl config file for temporary use. You can use `openssl version -a` to find out the location of the config file, ususally called  `openssl.cnf`.
